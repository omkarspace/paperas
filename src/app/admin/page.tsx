...
export default async function AdminDashboardPage() {
  ...
  const pendingReviews = await db.paper.count({
    where: {
      status: "UNDER_REVIEW",
    },
  });
  ...
  return (
    <div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Journal Issues</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-serif font-bold text-3xl">{pendingReviews}</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="space-y-2">
            {papersByStatus.map((item) => (
              <div key={item.status} className="flex justify-between">
                <span className="rounded-full px-2.5 py-0.5 text-xs font-medium border text-muted-foreground">{item.status.replace(/_/g, " ")}</span>
